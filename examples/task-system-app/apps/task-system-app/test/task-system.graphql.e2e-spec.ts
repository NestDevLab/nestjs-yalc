...
describe('Task System App GraphQL e2e', () => {
  ...
  it('supports JSON field round-trip in GraphQL', async () => {
    const jsonConfig = {
      provider: 'google',
      nested: {
        enabled: true,
    ...
    const createRes = await request(app.getHttpServer())
      ...
      .send({
        query: `
          mutation CreateSyncState($input: SyncStateCreateInput!) {
            TaskSystem_createSyncState(input: $input) {
              guid
              key
              value
            }
    ...
    expect(createRes.body.data.TaskSystem_createSyncState.value).toEqual(jsonConfig);

    const readRes = await request(app.getHttpServer())
      .post('/graphql')
      .send({
        query: `
          query GetSyncState($id: String!) {
            TaskSystem_getSyncState(ID: $id) {
              guid
              key
              value
            }
        ...
        `,
        variables: { id: syncStateGuid },
      })
      .expect(200);

    expect(readRes.body.errors).toBeUndefined();
    expect(readRes.body.data.TaskSystem_getSyncState.value).toEqual(jsonConfig);

    const updateRes = await request(app.getHttpServer())
      .post('/graphql')
      .send({
        query: `
          mutation UpdateSyncState($id: String!, $input: SyncStateUpdateInput!) {
            TaskSystem_updateSyncState(ID: $id, input: $input) {
              guid
              value
            }
          }
    ...
    expect(updateRes.body.data.TaskSystem_updateSyncState.value.nested.retries).toBe(5);
  });

  it('exposes JSON fields on GraphQL grids', async () => {
    const res = await request(app.getHttpServer())
      .post('/graphql')
      .send({
        query: `
          query SyncStateGrid {
            TaskSystem_getSyncStateGrid(
              sorting: [{ colId: "key", sort: ASC }]
            ) {
              pageData { count }
              nodes {
                guid
    ...
    expect(res.body.errors).toBeUndefined();
    const node = res.body.data.TaskSystem_getSyncStateGrid.nodes.find(
      (item: any) => item.guid === syncStateGuid,
    );
    expect(node).toBeDefined();
    expect(node.value.provider).toBe('google');
    expect(node.value.nested.retries).toBe(5);
  });

  it('supports JSON nested property sorting when exposed by the framework', async () => {
    const res = await request(app.getHttpServer())
      .post('/graphql')
      .send({
        query: `
          query SyncStateJsonSort {
            TaskSystem_getSyncStateGrid(
              sorting: [{ colId: "value.nested.retries", sort: ASC }]
            ) {
              pageData { count }
              nodes { guid key value }
            }
          }
        `,
      })
    ...
    if (res.body.errors) {
      ...
      expect(message).toContain('value.nested.retries');
      return;
    }

    expect(res.body.data.TaskSystem_getSyncStateGrid.pageData.count).toBeGreaterThanOrEqual(1);
    const node = res.body.data.TaskSystem_getSyncStateGrid.nodes.find(
      (item: any) => item.guid === syncStateGuid,
    );
    expect(node).toBeDefined();
