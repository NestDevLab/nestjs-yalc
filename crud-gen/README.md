...
- REST helpers: `CGQueryArgs`, pagination DTOs, structured JSON query parsing for `sorting`/`filters`, Swagger response helper, `crudRestControllerFactory` to generate full CRUD controllers (list/getById/create/update/delete) wired to your `GenericService`, with optional `readonly` and per-mutation toggles
