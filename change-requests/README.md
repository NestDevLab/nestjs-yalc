...
Add `ChangeRequestEntity` and `ChangeRequestItemEntity` to the host DataSource entities, and `CreateChangeRequests1727164800000` to its migrations. Disable TypeORM `synchronize` in production. Import `TypeOrmModule.forRoot(...)`, `EventEmitterModule.forRoot()`, and an `EventModule.forRootAsync(...)` configuration that exports `YalcEventService`, then import `ChangeRequestModule.forRoot({ adapters: [adapter], imports: [eventsModule], dataSource: 'settings' })`. Omit `dataSource` for the default connection. `forRootAsync({ imports, inject, useFactory, dataSource })` also accepts adapter factories. `ChangeRequestAdapterRegistry.register(adapter)` adds or replaces a target at runtime. The host must provide `YalcEventService` in the module's injection scope via an exported module in `imports`.
...
The migration uses a plain unique index and a foreign key with cascade deletion. It selects `datetime` on SQLite and `timestamp` on Postgres. Postgres support is covered by the migration type test only; no live Postgres database run was made. Values and source metadata use TypeORM `simple-json`; adapters should return JSON compatible values. The SHA-256 hash uses recursive, sorted JSON keys, so object key order does not affect stale detection. The service emits `change-request.created`, `updated`, `approved`, `rejected`, `withdrawn`, `stale`, and `publish_failed` with IDs and counts only. Publication failures log the adapter cause internally without emitting it as an event; `publish_error` stays client safe. The host remains responsible for authorization of decision operations and for publication side effects inside `apply`.
